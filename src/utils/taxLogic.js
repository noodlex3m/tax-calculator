	...
	if (system === "simplified") {
		if (group === "3") {
			if (numIncome > CALCULATED_CONSTANTS.INCOME_LIMIT_GROUP_3) {
				...
				tax =
					CALCULATED_CONSTANTS.INCOME_LIMIT_GROUP_3 *
					TAX_CONSTANTS_2025.GROUP_3_TAX_RATE;
			} else {
...
export function isMinIncomeForVat(income) {
	return parseFloat(income) > TAX_CONSTANTS_2025.MIN_INCOME_FOR_VAT; // у разі перевищення суми одержаного доходу на 1000000 грн на загальній системі
}
